#include "processsearchpattern1task.h"

ProcessSearchPattern1Task::ProcessSearchPattern1Task(int taskId, QObject *receiver)
    : m_taskId(taskId), m_receiver(receiver)
{
    setAutoDelete(true); // Автоматическое удаление после выполнения
}

void ProcessSearchPattern1Task::run()
{
...
//    }
    ...
    for(QStringList::iterator readIt = TotalGroups.begin(); readIt != TotalGroups.end(); ++readIt)
    {
        ...
        if(Progress < 100)
        {
            QMetaObject::invokeMethod(
                        m_receiver, "updateProgressSearchPattern1Task",
                        Qt::QueuedConnection,
                        Q_ARG(int, Progress));
        }
    ...
    qDebug() << "searchPattern1():FileLength=" << FileLength;

    QString fileName = cIniFile::pattern1StringListFilePath;
    QFile file(fileName);
